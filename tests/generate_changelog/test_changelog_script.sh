...
PROJECT_ROOT="$(cd "$(dirname "$(dirname "$TEST_DIR")")" && pwd)"
SCRIPT_DIR="$PROJECT_ROOT/src"
MAIN_CHANGELOG="$TEST_DIR/temp/CHANGELOG.md"
...
USAGE:
    test_changelog_script.sh [OPTIONS] [VERSION] [TEXT]
...
EXAMPLES:
    test_changelog_script.sh -y
    test_changelog_script.sh 1.2.3 -y
    test_changelog_script.sh --all-versions -y
    test_changelog_script.sh 1.2.3 "Manual changelog text"
EOF
}
